...
def check_valuable_nn(features_list,layers,device,train_x,target_num, training=False):
    
    if len(features_list) > target_num:
        var_list = []
        for i in range(0,len(features_list)):
            
            local_layers = layers[i].to(device)
            local_feature = features_list[i]
            
            if  len( train_x[:,local_feature].shape ) == 1:
                local_var = torch.var( local_layers( train_x[:,local_feature].reshape(-1,1),training ).sum(dim=1) )
                #local_var = (torch.mean( local_layers( train_x[:,local_feature].reshape(-1,1) ).sum(dim=1) ))**2
            else:
                local_var = torch.var( local_layers( train_x[:,local_feature] , training ).sum(dim=1)  ) 
                #local_var = (torch.mean( local_layers( train_x[:,local_feature] ).sum(dim=1) )**2)
            
            var_list.append(local_var.cpu().detach().item())
            
        
        if np.min(var_list) < 1*np.max(var_list):    
            min_index = np.argmin(var_list)
            min_feature = features_list[min_index]
            min_layer = layers[min_index]
            
            layers.remove( min_layer )
            features_list.remove(min_feature)  
            
            print(f"Remove component : {min_feature}")
        
        else:
            print("There is no component which is noninformative")
