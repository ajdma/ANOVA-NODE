...
class ANOVA_ODST_SCALE(ModuleWithInit):
    def __init__(self,
                 in_features,
                 num_trees, 
                 tree_dim=1,
        ...
        if len(component_list) == 0:
            ...
            if max_order ==2:
                component_list.extend(   list( itertools.combinations(component_list,2) )   )

            total_num_trees = len(component_list) * num_trees
        
        else:
            total_num_trees = len(component_list) * num_trees
            
        self.feature_selectors_param = torch.zeros([total_num_trees,in_features])
        for i in range(0,len(component_list)):
                
             self.feature_selectors_param[i*num_trees:(i+1)*num_trees,component_list[i]] = 1.0

        self.feature_selectors_param= self.feature_selectors_param.reshape(total_num_trees,in_features,1)
        ...
        self.device = device
        self.depth, self.num_trees, self.tree_dim = depth, num_trees, tree_dim
        self.total_num_trees = total_num_trees
        ...
        #FixME
        #self.response_ = nn.Parameter(torch.zeros([self.total_num_trees , self.tree_dim]), requires_grad=True)
        self.response_ = nn.Parameter(torch.zeros([self.total_num_trees , self.depth, self.tree_dim]), requires_grad=True)
        initialize_response_(self.response_)
        
        
        self.feature_selection_logits = nn.Parameter(
            torch.zeros([in_features, self.total_num_trees , self.depth]), requires_grad=True
        )
        initialize_selection_logits_(self.feature_selection_logits)

        self.feature_thresholds = nn.Parameter(
            torch.full([self.total_num_trees , self.depth], float('nan'), dtype=torch.float32), requires_grad=True
    ...
    def forward(self, input):
        ...
        feature_selectors  = torch.zeros([self.depth,self.total_num_trees ,self.depth]).to(input.device)
        ...
        bins = bins * self.feature_selectors_param.to(input.device)
        ...
        b = torch.ones(a.shape).to(input.device)

        a = a.reshape(a.shape[0],a.shape[1],1)
        b = b.reshape(a.shape[0],a.shape[1],1)
        
        weight = bins * torch.concat([b,a],axis=-1)
        ...
        #...
        new_response_ = self.response_.prod(axis=1)
        
        output = torch.einsum('ab,bd->abd', weight, new_response_)
    ...
    def initialize(self, input, eps=1e-6):
        ...
        with torch.no_grad():
            ...
            percentiles_q = 100 * np.random.beta(self.threshold_init_beta, self.threshold_init_beta,
                                                 size=[self.total_num_trees, self.depth])
            ...
            ).view(self.total_num_trees, self.depth)
...
class ODST(ModuleWithInit):
    def __init__(self, in_features, num_trees, tree_dim=1,
                 ...
                 threshold_init_beta=1.0, threshold_init_cutoff=1.0, device= "cpu" ):
        super().__init__()
    ...
    def forward(self, input):
        ...
        weight = weight.sum(axis=-1).prod(axis=-1)
        
        output = torch.einsum('ab,bd->abd', weight, self.response_)
